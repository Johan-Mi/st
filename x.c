...
xinit(int cols, int rows)
{
	...
	dc.ugc = XCreateGC(xw.dpy, XftDrawDrawable(xw.draw), None, NULL);
...
xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y)
{
	...
	if (base.mode & ATTR_UNDERLINE) {
		...
		if (base.ustyle != 3) {
			...
			XFillRectangle(xw.dpy, XftDrawDrawable(xw.draw), dc.ugc, winx,
			...
			XDrawArcs(xw.dpy, XftDrawDrawable(xw.draw), dc.ugc, arcs, narcs);
			...
			*/
			...
			if (npoints >= 3) {
				...
				// Draw the lines
				XDrawLines(xw.dpy, XftDrawDrawable(xw.draw), dc.ugc, points, npoints,
						CoordModePrevious);
				...
				if ( ((win.ch / (widthThreshold/2)) % 2)) {
					...
					XDrawLines(xw.dpy, XftDrawDrawable(xw.draw), dc.ugc, points,
